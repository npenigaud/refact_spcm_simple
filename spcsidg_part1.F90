...
#if defined(_OPENACC)
!$acc parallel private(im,ista,klx,ise,pas,pbs,pcs,entree,sortie,jlb,decalage,reste) default(none)
...
do jmloc=kmlocsta,kmlocend
  do compteurb=1,(ydgeometry%yrdimv%nflevg-1)/bloclev+1
      ...
      !!  ISE=ISTA+2*(JN-IM)
      !!  do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
      ...
      IF (IM > 0) THEN
        ...
        IF (KLX >= 3) THEN
          ...
          do jlb=1,(klx-3)/tbloc+1  !!klx-3+1 elements, de 3 a klx
            ...
            !$acc loop vector private(compteurc,compteur,decalage1,ise)
            do jl=decalage+1,min(decalage+tbloc+2,klx)
              ise=ista+2*(jl-1)
              do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
                compteur=compteurc+(compteurb-1)*bloclev
                decalage1=(compteur-1)*(ydgeometry%yrdim%nsmax+1-im)
                ...
                do ji=1,2
                  entree(jl-decalage,compteurc,ji)=psdivp(ise+ji-1,compteur)!!zsdivpl(jl,compteur,ji,jmloc)
                enddo
              enddo
            enddo
            if (jlb==1) then
              ...
              ise=ista+2*(decalage+1-1)
              !$acc loop vector private(compteur) collapse(2)
              do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
                do ji=1,2
                  compteur=compteurc+(compteurb-1)*bloclev
                  sortie(1,compteurc,ji)=pspdivp(ise+ji-1,compteur)!!zspdivpl(decalage+1,compteur,ji,jmloc)
                  sortie(2,compteurc,ji)=pspdivp(ise+ji+1,compteur)!!zspdivpl(decalage+2,compteur,ji,jmloc)
                enddo
              enddo
            endif
            ...
            do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
              ...
              enddo
            enddo
            !$acc loop vector private(compteurc,ji,ise)
            do jl=decalage+1,min(decalage+tbloc+2,klx)
              ise=ista+2*(jl-1)
              do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
                do ji=1,2
                  pspdivp(ise+ji-1,compteurc+(compteurb-1)*bloclev)=sortie(jl-decalage,compteurc,ji)
        ...
        ELSE
          ...
          do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
            do ji=1,2
              compteur=compteurc+(compteurb-1)*bloclev
              decalage1=(compteur-1)*(ydgeometry%yrdim%nsmax+1-im)
              pspdivp(ise+ji-1,compteur)=psdivp(ise+ji-1,compteur)/param_mxture(decalage1+1,jmloc,1)
!!              zspdivpl(1,compteur,ji,jmloc)=zsdivpl(1,compteur,ji,jmloc)/param_mxture(decalage1+1,jmloc,1)
              IF (KLX >= 2) THEN
                pspdivp(ise+ji+1,compteur)=(psdivp(ise+ji+1,compteur)&
                 &-param_mxture(decalage1+1,jmloc,2)*pspdivp(ise+ji-1,compteur))/param_mxture(decalage1+2,jmloc,1)
...
!!       !$acc loop vector private(compteurc,compteur,ji,ise)
!!       do jl=1,klx
!!         ise=ista+2*(jl-1)
!!         do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
!!           compteur=compteurc+(compteurb-1)*bloclev
!!           do ji=1,2
!!             psdivp(ise+ji-1,compteur)=pspdivp(ise+ji-1,compteur)
...
!!       ENDDO
       ...
       IF (KLX >= 3) THEN
         ...
         do jlb=(klx-3)/tbloc+1,1,-1  !!klx-3+1 elements, de klx-2 a 1
           ...
           !$acc loop vector private(compteur,compteurc,decalage1,ji,ise)
           do jl=decalage+tbloc+2,max(decalage+1,1),-1
             ...
             do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
               ...
               pcs(jl-decalage,compteurc)=param_mxture(decalage1+jl,jmloc,3)
               do ji=1,2
                 entree(jl-decalage,compteurc,ji)=pspdivp(ise+ji-1,compteur)!!zsdivpl(jl,compteur,ji,jmloc)
               enddo
             enddo
           enddo
           if (jlb==(klx-3)/tbloc+1) then
              ...
              ise=ista+2*(decalage+tbloc+1-1)
              !$acc loop vector private(compteur) collapse(2)
              do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
                do ji=1,2
                  compteur=compteurc+(compteurb-1)*bloclev
                  sortie(tbloc+1,compteurc,ji)=pspdivp(ise+ji-1,compteur)!!zspdivpl(decalage+tbloc+1,compteur,ji,jmloc)
                  sortie(tbloc+2,compteurc,ji)=pspdivp(ise+ji+1,compteur)!!zspdivpl(decalage+tbloc+2,compteur,ji,jmloc)
                enddo
              enddo
           endif
           !$acc loop vector private(jl) collapse(2)
...
!!             zspdivpl(KLX,compteur,ji,jmloc)=zsdivpl(KLX,compteur,ji,jmloc)
             IF (KLX >= 2) THEN
               pspdivp(ise+ji-1,compteur)=pspdivp(ise+ji-1,compteur)&
                &-param_mxture(decalage1+klx-1,jmloc,2)/param_mxture(decalage1+klx-1,jmloc,1)*pspdivp(ise+ji+1,compteur)
        ...
        IF (KLX >= 3) THEN
          ...
          do jlb=1,(klx-3)/tbloc+1  !!klx-3+1 elements, de 3 a klx
            ...
            do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
              ...
              enddo
            enddo
            !$acc loop vector private(compteurc,ise)
            do jl=decalage+1,min(decalage+tbloc+2,klx)
              ise=ista+2*(jl-1)
              do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
                pspdivp(ise+ji-1,compteurc+(compteurb-1)*bloclev)=sortie(jl-decalage,compteurc,ji)
...
!!       !$acc loop vector private(compteurc,compteur,ise)
!!       do jl=1,klx
!!         ise=ista+2*(jl-1)
!!         do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
!!           compteur=compteurc+(compteurb-1)*bloclev
...
!!         zspdivpl(KLX,compteur,ji,jmloc)=zsdivpl(KLX,compteur,ji,jmloc)
         IF (KLX >= 2) THEN
           pspdivp(ise+ji-1,compteur)=pspdivp(ise+ji-1,compteur)&
             &-param_mxture(decalage1+klx-1,jmloc,4)*pspdivp(ise+ji+1,compteur)
!!           zspdivpl(KLX-1,compteur,ji,jmloc)=zsdivpl(KLX-1,compteur,ji,jmloc)&
!!             &-param_mxture(decalage1+klx-1,jmloc,4)*zspdivpl(KLX,compteur,ji,jmloc)
     ...
     ENDIF
          ...
          do jn=1,ydgeometry%yrdim%nsmax+1-im
            ise=ista+2*(jn-1)
            do compteurc=1,min(bloclev,ydgeometry%yrdimv%nflevg-(compteurb-1)*bloclev)
              compteur=compteurc+(compteurb-1)*bloclev
              pspdivp(ise+ji-1,compteur)=psdivp(ise+ji-1,compteur)
