...
CONTAINS
#if defined(_OPENACC)
SUBROUTINE EXCHANGE_MS (YDGEOMETRY, YDLIST, zbuf_m,zbuf_s,KDIR, LDFULLM)
#else
...
!$acc data present(ydmp,ydlist,zbuf_m,zbuf_s,ydlist%yl3d,ydlist%yl2d) create(iptrsv)
LLFULLM=.FALSE.
...
SELECT CASE (KDIR)
  CASE (NEXCHANGE_MTOS) 
    CALL EXCHANGE (YDGEOMETRY, zbuf_m,zbuf_s,ISPEC2V, IPTRSV, YDLIST, KDIR, MTAGMS, &
                 & ISIZEMAX_M, INPROC_M, INRANK_M, IPEER_M, ISIZE_M, MESSAGE_M, &
                 & ISIZEMAX_S, INPROC_S, INRANK_S, IPEER_S, ISIZE_S, MESSAGE_S)
  CASE (NEXCHANGE_STOM)
    CALL EXCHANGE (YDGEOMETRY, zbuf_s, zbuf_m,ISPEC2V, IPTRSV, YDLIST, KDIR, MTAGSM, &
                 & ISIZEMAX_S, INPROC_S, INRANK_S, IPEER_S, ISIZE_S, MESSAGE_S, &
                 & ISIZEMAX_M, INPROC_M, INRANK_M, IPEER_M, ISIZE_M, MESSAGE_M)
  CASE DEFAULT
...
#if defined(_OPENACC)
SUBROUTINE EXCHANGE (YDGEOMETRY, zbufsend,zbufrecv,KSPEC2V, KPTRSV, YDLIST, KDIR, KTAG,     &
...
!$acc data present(ydlist,ydlist%yl3d,ydlist%yl2d,zbufsend,zbufrecv,nflevl,npsp,kptrsv)
! * Recv loop .................................................
...
!$acc data present(nflevl,ydlist,ydlist%yl3d,ydlist%yl2d,pbuf_m)
...
!!!$acc parallel private(jfld,ioff) default(none)
IOFF=0
!!!$acc loop seq
DO JFLD = 1, YDLIST%N2D
  IF ((NPSP == 1) .OR. &
...
#if defined(_OPENACC)
!$acc parallel private(ispe,ipos,jfld) default(none)
!$acc loop gang vector
#else
...
!$acc data present(ydlist,ydlist%yl2d,ydlist%yl3d,pbuf_s)
...
!!!$acc parallel private(ioff,jfld) default(none)
IOFF = 0
!!!$acc loop seq
DO JFLD = 1, YDLIST%N2D
...
!!!$acc data present(yl2d,psp,pspg,yl2d%zsp,yl2d%zspg)
  YL2D%CNAME = CDNAME
  YL2D%ZSP   => PSP
  YL2D%ZSPG  => PSPG
  IF (PRESENT (LDBCAST)) YL2D%LBCAST = LDBCAST
!$acc enter data attach(yl2d%zsp,yl2d%zspg)
!!!$acc end data
END ASSOCIATE
!!!$acc end data
999 CONTINUE
...
!!!$acc data present(yl3d,psp,pspg,yl3d%zsp,yl3d%zspg)
  YL3D%CNAME = CDNAME
  YL3D%ZSP   => PSP
  YL3D%ZSPG  => PSPG
!$acc enter data attach(yl3d%zspg,yl3d%zsp)
!!!$acc end data
END ASSOCIATE
!!!$acc end data
999 CONTINUE
...
DO JFLD = 1, SIZE (PSP, 2)
  ...
  ASSOCIATE (YL2D => YDLIST%YL2D (YDLIST%N2D))
!!!$acc data present(yl2d,psp,pspg)
...
DO JFLD = 1, SIZE (PSP, 3)
  ...
  ASSOCIATE (YL3D => YDLIST%YL3D (YDLIST%N3D))
!!!$acc data present(yl3d,psp,pspg)
    WRITE (YL3D%CNAME, '(A,".",I4.4)') JFLD
    YL3D%ZSP   => PSP (:,:,JFLD)
    YL3D%ZSPG  => PSPG (:,:,JFLDG)
    !$acc enter data attach(yl3d%zsp,yl3d%zspg)
!!!$acc end data
  END ASSOCIATE
