...
CLFILE = 'fort.55.'//TRIM (CLPROC)

OPEN (55, FILE=TRIM (CLFILE), FORM='FORMATTED')
CALL SPCM_SIMPLE (YDGEOMETRY,YDMODEL,PSPSP,PSPVOR,PSPDIV,PSPT,PSPSPD,PSPSVD)
CLOSE (55)

IF (MYPROC == 3) THEN
  WRITE (*, '(E30.20)') PSPT (1, 5661)
ENDIF
...
IF (MYPROC == 3) THEN
  WRITE (*, *) ' COUNT = ', COUNT (PSPT /= ZSPT)
  DO JLEV = 1, YDGEOMETRY%YRDIMV%NFLEVL
    WRITE (*, *) JLEV, ' COUNT = ', COUNT (PSPT (JLEV, :) /= ZSPT (JLEV, :))
  ENDDO

  JLEV = 1
  DO JSP = 1, YDGEOMETRY%YRDIM%NSPEC2
    IF (PSPT (JLEV, JSP) /= ZSPT (JLEV, JSP)) THEN
      WRITE (*, *) JSP, PSPT (JLEV, JSP), ZSPT (JLEV, JSP)
    ENDIF
  ENDDO

  WRITE (*, '(2E30.20)') PSPT (1, 5661), ZSPT (1, 5661)
ENDIF
